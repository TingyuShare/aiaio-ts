...
export class ChatDatabase {
  ...
  private deriveKey(): Buffer {
    const code = this.getAccessCode() || "aiaio-ts";
  ...
  private decryptApiKey(encoded: string): string {
    if (!encoded) return "";
    try {
      const key = this.deriveKey();
  ...
  getDefaultProvider(): Provider | undefined {
    ...
    if (p) p.api_key = this.decryptApiKey(p.api_key);
    return p;
  }

  getAllProviders(): Provider[] {
    const providers = this.queryAll("SELECT * FROM providers ORDER BY name") as unknown as Provider[];
    for (const p of providers) p.api_key = this.decryptApiKey(p.api_key);
    return providers;
  }
  ...
  addProvider(provider: { name: string; temperature?: number; top_p?: number; reasoning_effort?: string; use_for_summarization?: boolean; host: string; api_key?: string; api_key_header?: string }): number {
    this.db.run(
      ...
      [provider.name, provider.temperature ?? 1.0, provider.top_p ?? 0.95, provider.reasoning_effort ?? "none", provider.use_for_summarization ? 1 : 0, provider.host, this.encryptApiKey(provider.api_key ?? ""), provider.api_key_header ?? "Authorization"]
    );
    const r = this.db.exec("SELECT last_insert_rowid()");
    this.save();
  ...
  updateProvider(providerId: number, provider: { name: string; temperature?: number; top_p?: number; reasoning_effort?: string; use_for_summarization?: boolean; host: string; api_key?: string; api_key_header?: string }): boolean {
    this.db.run(
      ...
      [provider.name, provider.temperature ?? 1.0, provider.top_p ?? 0.95, provider.reasoning_effort ?? "none", provider.use_for_summarization ? 1 : 0, provider.host, this.encryptApiKey(provider.api_key ?? ""), provider.api_key_header ?? "Authorization", providerId]
    );
    this.save();
    return true;
  }
